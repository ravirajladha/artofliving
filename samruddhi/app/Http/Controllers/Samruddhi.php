...
// use Alert;
//use Auth;
class Samruddhi extends Controller
{
    //
    ...
    function index()
    {
        ...
        if (Session::has('user'))
        {
            if (Session::get('rexkod_apex_user_type') == "admin" || Session::get('rexkod_apex_user_type') == "itadmin"  )
            {
    ...
    function add_asset_select()
    {
        ...
        // ];
        $firstOptions = Category::where('active_status',0)->get();
  return view('add_asset_select', compact('firstOptions'));
    ...
    function add_user()
    {
        ...
        if(Session('rexkod_apex_user_type')=='admin' || Session('rexkod_apex_user_type')=='itadmin'){
            $privilege = Privilege::all();
           
        }
        else{
            // $privilege = Privilege::where('admin_privilege',0)
            // ->get();
            return redirect('/access_denied');
        }
        // $privilege = Privilege::where('admin_privilege',0)->get();


        $users = Auth::where('type', 'manager')->get();
        $locations = Locations::where('created_by', Session('rexkod_apex_user_id'))->get();
    ...
    function all_assets()
    {
        if(Session('rexkod_apex_user_type')=='admin' || Session('rexkod_apex_user_type')=='itadmin'){
            ...
            $assets = Assets::where('created_by',Session('rexkod_apex_user_id'))
    ...
    function incidents()
    {
        if(Session::get('rexkod_apex_user_type') == "admin" || Session::get('rexkod_apex_user_type') == "owner"){
            $incidents = Incidents::all();
        }
        else{
            $incidents = Incidents::where('user_id',Session('rexkod_apex_user_id'))->get();
        }
    ...
    function locations()
    {
        if(Session('rexkod_apex_user_type')=='admin' || Session('rexkod_apex_user_type')=='itadmin'){
            ...
            $locations = Locations::where('created_by',Session('rexkod_apex_user_id'))
            ->orderBy('id', 'DESC')
            ->get();
        }
    ...
    function properties()
    {
        if(Session('rexkod_apex_user_type')=='admin' || Session('rexkod_apex_user_type')=='itadmin'){
            ...
            $property = Property::where('created_by',Session('rexkod_apex_user_id'))
            ->orderBy('id', 'DESC')
            ->get();
        }
    ...
    function asset_audits()
    {
            ...
            else if (Session::get('rexkod_apex_user_type') == "itadmin") {
    ...
    function reports(Request $req)
    {

        $category = Category::all();
        $subcategory = Subcategory::all();
        $type =  Auth::where('type', $req->type)->get();
    ...
    function add_asset_audit($location_id)
    {
        ...
        if(Session('rexkod_apex_user_type')=='admin' || Session('rexkod_apex_user_type')=='itadmin' || Session('rexkod_apex_user_type')=='auditor'){
            $locations = Locations::orderBy('id', 'DESC')
            ->get();
        }
        else{
        ...
        if(Session('rexkod_apex_user_type')=='admin' || Session('rexkod_apex_user_type')=='itadmin' ||
        Session('rexkod_apex_user_type')=='auditor'){
            $get_users = Auth::where('type', 'auditor')
            ->orderBy('id', 'DESC')
            ->get();
        }
        // else if (Session('rexkod_apex_user_type')=='auditor'){
        //     $get_users = Auth::where('id',Session('rexkod_apex_user_id'))
        //     ->where('type', 'auditor')
        //     ->orderBy('id', 'DESC')
        //     ->get();
        // }
    ...
    function create_asset_audit(Request $req)
    {
        ...
        session()->put('failed','Minimum 1 asset is required for the audit');
        ...
        session()->put('success','Asset Audit Created');
  ...
  return $output;
    ...
    function tickets()
    {
        if(Session::get('rexkod_apex_user_type') == "admin" || Session::get('rexkod_apex_user_type') == "owner"){
            ...
            $tickets = Tickets::where('user_id',Session('rexkod_apex_user_id'))->get();
        }
    ...
    {
        if(Session('rexkod_apex_user_type')=='admin' || Session('rexkod_apex_user_type')=='itadmin'){
            $users = Auth::orderBy('id', 'DESC')
            ->get();
        }
        else{
            $users = Auth::where('created_by',Session('rexkod_apex_user_id'))
            ->orderBy('id', 'DESC')
            ->get();
            // return redirect('/access_denied');
    ...
    function user($id)
    {
        ...
        $assetsc = Assets::where('owner_id', $id)->orderBy('id','desc')->get();
    ...
    {
        $user = Auth::where('manager_id',Session('rexkod_apex_user_id'))->get();

        // dd(Session('rexkod_apex_user_id'));
        // dd($user->id);
        // print_r($user);
        // die;
    ...
    function asset($id)
    {
        ...
        $buildings = Buildings::all();
        $cabins = Cabins::all();
        $ticket_types = Ticket_types::where('active_status', 0)->get();
        $document_types = Document_types::where('active_status', 0)->get();
        $incident_types = Incident_types::where('active_status', 0)->get();
        $asset =  Assets::where('id', $id)->first();
        $assigns =  Assigns::where('asset_id', $id)
        ->orderBy('id','DESC')
        ->get();
        // dd($assigns);
    ...
    function create_location(Request $req)
    {
        ...
        $locations->created_by = Session('rexkod_apex_user_id');
    ...
    //    die;
        ...
        $auth->status = '1';
        $auth->created_by = Session('rexkod_apex_user_id');
    ...
    function create_property(Request $req)
    {
        ...
        $property->created_by = Session('rexkod_apex_user_id');
    ...
    function create_property_audit(Request $req)
    {
        ...
        if(isset($req->property_id)){
            $property->property_id = implode(',', $req->property_id);


        }
        else{
        session()->put('failed','Minimum 1 property is required for the audit');
        ...
        $property->save();
        session()->put('success','Asset Property Created');
    ...
    // }
        ...
        if($user->active_status==1){
            session()->put('failed','User Deactivated');
            return redirect('/login');
        }
        else{
 ...
 function update_user(Request $req)
 {
    ...
    if (str_contains($req->password, 'script')) { session()->flush(); return redirect('login');}
     ...
     function create_state(Request $req)
     {
        ...
        $check_name = States::where('name',$req->state)->first();
        // dd($check_name);
        if ($check_name) {
     ...
     function create_document_type(Request $req)
     {
        ...
        $check_name = Document_types::where('name',$req->document)->first();
        if ($check_name) {
     ...
     function create_incident_types(Request $req)
     {
        ...
        $check_name = Incident_types::where('name',$req->incident_type)->first();
        if ($check_name) {
     ...
     function create_ticket_types(Request $req)
     {
        ...
        $check_name = Ticket_types::where('name',$req->ticket_type)->first();
        if ($check_name) {
    ...
    function create_subcategory(Request $req)
    {
        ...
        $check_name = Subcategory::where('name',$req->name)->first();
        if ($check_name) {

            session()->put('failed','Subcategory already exists');
            return redirect()->back();
        }
    ...
    function transfer_asset($id,Request $req)
    {
        ...
        $asset_id = $id;
            ...
            $notify->type= $type;
            $notify->user_id= $user_id;
            $notify->from_user_id= Session('rexkod_apex_user_id');
    ...
    function create_asset(Request $req)
    {
        ...
        $check_qr = Assets::where('qr',$req->qr)->first();
        if ($check_qr) {

            session()->put('failed','QR Code already exists');
            return redirect()->back();
        ...
        function check_isset($var)
        {
            ...
            $asset_new->created_by = Session('rexkod_apex_user_id');
    ...
    public function assign_response($nid, $aid, $status)
    {
            ...
            $assigns = Assigns::where('asset_id',$aid)->get();
            ...
            $assign =  Assigns::where('asset_id', $aid)->orderBy('id', 'desc')->first();
            $asset =  Assets::where('id', $assign->asset_id)->first();

            // dd($asset->id);

            $type = "transfer back";
            $notification = $asset->name . " is accepted";

            $notify= new Notifications;

            $notify->type= $type;
            $notify->user_id= $assign->from_user_id;
            $notify->manager_id= NULL;
            $notify->asset_id= $asset->id;
            $notify->notification= $notification;
            $notify->save();
    ...
    function subcategory_ds()
    {
       $fields = DB::table('information_schema.COLUMNS')
        ...
        $subcategory = Subcategory::orderBy('id', 'desc')->get();
        $category = Category::orderBy('id', 'desc')->get();
        $compliances = Compliances::where('active_status',0)
        ->orderBy('id', 'desc')
        ->get();
    ...
    function update_compliance(Request $req){
     ...
     function edit_subcategory_ds($id,Request $req)
     {
        $fields = DB::table('information_schema.COLUMNS')
        ->select('COLUMN_NAME')
        ->distinct()
        ...
        $data = [
         ...
         return view('/edit_subcategory_ds', ['data' => $data]);
     }
     ...
     function delete_subcategory(Request $req){
      ...
      function update_subcategory(Request $req){
        ...
        $compliances = implode(',', $compliances);
            ...
            $compliances = $user->compliances;
        }

        $user->fields=$fields;
        $user->compliances=$compliances;
        $user->save();
    ...
    function add_property_upload()
    {
        return view('/add_property_upload');
    }
    function audit_upload()
    {
        return view('/audit_upload');
    }
    ...
    function upload_users(Request $req){
        ...
        while(($line_check = fgetcsv($csvFile)) !== FALSE){
            ...
            for($i=0;$i<8;$i++){
              if (str_contains($line_check[$i], 'script')) { session()->flush(); return redirect('login');}
            }

            if($row_count){

            if(!$line_check[1]){
                 session()->put('failed',"'Name' missing at Line ".$row_count);
                 return redirect('/add_user_upload');
            }
            if(!$line_check[2]){
                 session()->put('failed',"'Email' missing at Line ".$row_count);
                 return redirect('/add_user_upload');
            }

            if(!$line_check[3]){
                session()->put('failed',"'Type' missing at Line ".$row_count);
                return redirect('/add_user_upload');
            }
            ...
            $array  = ['admin','itadmin','apex','manager','owner','auditor','employee','teacher'];
            if(!in_array($line_check[3],$array)){
                session()->put('failed',"'Wrong Type' at Line ".$row_count);
                return redirect('/add_user_upload');
            }
    ...
    function upload_audit(Request $req){
        // dd(empty($req->file('upload')));
        // echo "test";die;
        // Allowed mime types
        $csvMimes = array('text/x-comma-separated-values', 'text/comma-separated-values', 'application/octet-stream', 'application/vnd.ms-excel', 'application/x-csv', 'text/x-csv', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.msexcel', 'text/plain');
        ...
        while(($line_check = fgetcsv($csvFile)) !== FALSE){
            ...
            if(!$line_check[0]){
                 session()->put('failed',"'Audit ID' missing at Line ".$row_count);
        ...
        //    echo "test";die;
                ...
                $valid = 1;
                $audit_id = $line[0];
//                 $var =  Asset_audit_master::where('id',$audit_id)->get();
// if(count($var)>0){
//     its present
// }
// else{
// }
              ...
              //   die();
                $asset_id = $line[1];
                $notes = $line[2];
                $initial_condition = $line[3];
                $status = $line[4];
                $unassigned_asset = $line[5];
                $not_found = $line[6];
              

                $user=new Asset_audits;

                $user->audit_id = $audit_id;
                $user->asset_id = $asset_id;
                $user->notes = $notes;
                $user->initial_condition = $initial_condition;
                $user->status = $status;
                $user->unassigned_asset = $unassigned_asset;
      ...
      function upload_property(Request $req){
        // dd(empty($req->file('upload')));
        // echo "test";die;
        // Allowed mime types
        $csvMimes = array('text/x-comma-separated-values', 'text/comma-separated-values', 'application/octet-stream', 'application/vnd.ms-excel', 'application/x-csv', 'text/x-csv', 'text/csv', 'application/csv', 'application/excel', 'application/vnd.msexcel', 'text/plain');
      ...
      while(($line_check = fgetcsv($csvFile)) !== FALSE){
          ...
          if(!$line_check[1]){
               session()->put('failed',"'Name' missing at Line ".$row_count);
               return redirect('/add_property_upload');
          }
      ...
      $count=0;
        ...
        //    echo "test";die;
              ...
              //   die();
                $name = $line[1];
                $category = $line[2];
                $agreement_type = $line[3];
                $other_assests_on_property = $line[4];
                $purpose_of_this_property = $line[5];
                $address = $line[6];
                $pincode = $line[7];
                $district = $line[8];
                $state = $line[9];
                ...
                $cost = $line[14];
                $fmv = $line[15];
                $previous_owner = $line[16];
                $previous_owner_phn = $line[17];
                $laising_advocate_name = $line[18];
                $laising_advocate_phn = $line[19];
                $reg_number = $line[20];

                $applicable_tvc = $line[21];
                $notes_tvc = $line[22];

                $applicable_khata = $line[23];
                $notes_khata = $line[24];

                $applicable_patta = $line[25];
                $notes_patta = $line[26];

                $applicable_encumburance = $line[27];
                $notes_encumburance = $line[28];

                $applicable_survy_sketch = $line[29];
                $notes_survy_sketch = $line[30];

                $applicable_soiltest = $line[31];
                $notes_soiltest = $line[32];

                $applicable_ele_cls = $line[33];
                $notes_ele_cls = $line[34];

                $applicable_constr_appro = $line[35];
                $notes_constr_appro = $line[36];

                $applicable_tax_cls = $line[37];
                $notes_tax_cls = $line[38];

                $applicable_forest_cls = $line[39];
                $notes_forest_cls = $line[40];

                $applicable_court_docs = $line[41];
                $notes_court_docs = $line[42];

                $applicable_overall_legal_cls = $line[43];
                $notes_overall_legal_cls = $line[44];

                $applicable_trust_resol = $line[45];
                $notes_trust_resol = $line[46];

                $applicable_orig_rec_at_ho = $line[47];
                $notes_orig_recho = $line[48];

                $photos_and_video_notes = $line[49];
                $other_notes = $line[50];
                $additional_information = $line[51];
                ...
                $user->agreement_type=$agreement_type;
                $user->other_assests_on_property=$other_assests_on_property;
                $user->purpose_of_this_property=$purpose_of_this_property;
                ...
                $user->cost=$cost;
                $user->fmv=$fmv;
                $user->previous_owner=$previous_owner;
                $user->previous_owner_phn=$previous_owner_phn;
                $user->laising_advocate_name=$laising_advocate_name;
                $user->laising_advocate_phn=$laising_advocate_phn;
                $user->reg_number=$reg_number;
                $user->applicable_tvc=$applicable_tvc;
                $user->notes_tvc=$notes_tvc;
                $user->applicable_khata=$applicable_khata;
                $user->notes_khata=$notes_khata;
                $user->applicable_patta=$applicable_patta;
                $user->notes_patta=$notes_patta;
                $user->applicable_encumburance=$applicable_encumburance;
                $user->notes_encumburance=$notes_encumburance;
                $user->applicable_survy_sketch=$applicable_survy_sketch;
                $user->notes_survy_sketch=$notes_survy_sketch;
                $user->applicable_soiltest=$applicable_soiltest;
                $user->notes_soiltest=$notes_soiltest;
                $user->applicable_ele_cls=$applicable_ele_cls;
                $user->notes_ele_cls=$notes_ele_cls;
                $user->applicable_constr_appro=$applicable_constr_appro;
                $user->notes_constr_appro=$notes_constr_appro;
                $user->applicable_tax_cls=$applicable_tax_cls;
                $user->notes_tax_cls=$notes_tax_cls;
                $user->applicable_forest_cls=$applicable_forest_cls;
                $user->notes_forest_cls=$notes_forest_cls;
                $user->applicable_court_docs=$applicable_court_docs;
                $user->notes_court_docs=$notes_court_docs;
                $user->applicable_overall_legal_cls=$applicable_overall_legal_cls;
                $user->notes_overall_legal_cls=$notes_overall_legal_cls;
                $user->applicable_trust_resol=$applicable_trust_resol;
                $user->notes_trust_resol=$notes_trust_resol;
                $user->applicable_orig_rec_at_ho=$applicable_orig_rec_at_ho;
                $user->notes_orig_recho=$notes_orig_recho;
                $user->photos_and_video_notes=$photos_and_video_notes;
                $user->other_notes=$other_notes;
      ...
      while(($line_check = fgetcsv($csvFile)) !== FALSE){
          ...
          if(!$line_check[4]){
               session()->put('failed',"'Name' missing at Line ".$row_count);
               return redirect('/add_asset_upload');
          }
           ...
           // echo "test";die;
            // Get row line
              ...
              //   die();
                ...
                $invoice_number = $line[27];
                $qr = $line[28];
                
                $tenure = $line[29];
                $land_area = $line[30];
                $cost_to_trust = $line[31];
                $buildings = $line[32];
                $wdv = $line[33];
                $original_documents = $line[34];
                $receipt_date = $line[35];
                $receipt_via = $line[36];
                $file_no = $line[37];
                $purpose = $line[38];
                $remark = $line[39];
                $initial_condition = $line[40];
                $address = $line[41];
                $city = $line[42];
                $state = $line[43];
                $pincode = $line[44];
                ...
                $user->cost_to_trust =$cost_to_trust;
                $user->buildings =$buildings;
                $user->wdv =$wdv;
                $user->original_documents =$original_documents;
                $user->receipt_date =$receipt_date;
                $user->receipt_via =$receipt_via;
                $user->file_no =$file_no;
    ...
    {   
        ...
        $dis = Auth::find($user_id);
        $dis->active_status=$status;
        $dis->save();

        if (($dis->active_status == 0)) {
            session()->put('success','User is Active');
            return redirect('/users');
        } elseif (($dis->active_status == 1)) {
            session()->put('success','User is Deactivated');
            return redirect('/users');
        } else {
            session()->put('success','Some Error Occured');
            return redirect('/users');
        }
    }

        function change_status_cat($user_id, $status)
    {   

        $dis = Category::find($user_id);
        $dis->active_status=$status;
        $dis->save();

        if (($dis->active_status == 0)) {
            session()->put('success','Category is Active');
            return redirect('/category_ds');
        } elseif (($dis->active_status == 1)) {
            session()->put('success','Category is Deactivated');
            return redirect('/category_ds');
        } else {
            session()->put('success','Some Error Occured');
            return redirect('/category_ds');
        ...
        function change_status_state($user_id, $status)
    {   

        $dis = States::find($user_id);
        $dis->active_status=$status;
        $dis->save();

        if (($dis->active_status == 0)) {
            session()->put('success','Apex is Active');
            return redirect('/states');
        } elseif (($dis->active_status == 1)) {
            session()->put('success','Apex is Deactivated');
            return redirect('/states');
        } else {
            session()->put('success','Some Error Occured');
            return redirect('/states');
        ...
        function change_status_document($user_id, $status)
    {   

        $dis = Document_types::find($user_id);
        $dis->active_status=$status;
        $dis->save();

        if (($dis->active_status == 0)) {
            session()->put('success','Document Type is Active');
            return redirect('/document_types');
        } elseif (($dis->active_status == 1)) {
            session()->put('success','Document Type is Deactivated');
            return redirect('/document_types');
        } else {
            session()->put('success','Some Error Occured');
            return redirect('/document_types');
        ...
        function change_status_incident($user_id, $status)
    {   

        $dis = Incident_types::find($user_id);
        $dis->active_status=$status;
        $dis->save();

        if (($dis->active_status == 0)) {
            session()->put('success','Incident Type is Active');
            return redirect('/incident_types');
        } elseif (($dis->active_status == 1)) {
            session()->put('success','Incident Type is Deactivated');
            return redirect('/incident_types');
        } else {
            session()->put('success','Some Error Occured');
            return redirect('/incident_types');
        ...
        function change_status_ticket($user_id, $status)
    {   

        $dis = Ticket_types::find($user_id);
        $dis->active_status=$status;
        $dis->save();

        if (($dis->active_status == 0)) {
            session()->put('success','Ticket Type is Active');
            return redirect('/ticket_types');
        } elseif (($dis->active_status == 1)) {
            session()->put('success','Ticket Type is Deactivated');
            return redirect('/ticket_types');
        } else {
            session()->put('success','Some Error Occured');
            return redirect('/ticket_types');
        ...
        function change_status_compliance($user_id, $status)
    {   

        $dis = Compliances::find($user_id);
        $dis->active_status=$status;
        $dis->save();

        if (($dis->active_status == 0)) {
            session()->put('success','Compliance is Active');
            return redirect('/compliance_ds');
        } elseif (($dis->active_status == 1)) {
            session()->put('success','Compliance is Deactivated');
            return redirect('/compliance_ds');
        } else {
            session()->put('success','Some Error Occured');
            return redirect('/compliance_ds');
        ...
        function change_status_subcat($user_id, $status)
    {   

        $dis = Subcategory::find($user_id);
        $dis->active_status=$status;
        $dis->save();

        if (($dis->active_status == 0)) {
            session()->put('success','Subcategory is Active');
            return redirect('/subcategory_ds');
        } elseif (($dis->active_status == 1)) {
            session()->put('success','Subcategory is Deactivated');
            return redirect('/subcategory_ds');
        } else {
            session()->put('success','Some Error Occured');
            return redirect('/subcategory_ds');
    ...
    function unassigned_asset(Request $req,$id)
    {
        $data = [
            'audit_id' => $id,
        ];
        ...
        return view('/unassigned_asset',['data' => $data]);
    }

    function create_unassigned_asset(Request $req)
    {
//         if (str_contains($req->asset_id, 'script')) { session()->flush(); return redirect('login');}
//         $audit_id= $req->audit_id;
//         // dd($audit_id);
//         $unassigned = Asset_audit_master::find($audit_id);

//         $u = $unassigned->unassigned_asset;
//         $unassigned->unassigned_asset = $u.','.$req->asset_id;

//         $asset_id = $unassigned->asset_id;
//         $unassigned->asset_id = $asset_id.','.$req->asset_id;

//         $unassigned->save();

//         $u = new Asset_audits;
//         $u->unassigned_asset=1;
//         $u->asset_id= $req->asset_id;
//         $u->audit_id= $req->audit_id;
//         $u->save();
...
// ========================================
        ...
        $asset = Assets::where('qr',$req->qr)->first();
        // dd($asset->id);
        if(isset($asset->id)){
        ...
        $u = $unassigned->unassigned_asset;
        $unassigned->unassigned_asset = $u.','.$asset->id;

        $asset_id = $unassigned->asset_id;
        // dd($asset->id);
        $unassigned->asset_id = $asset_id.','.$asset->id;
        ...
        return redirect('update_asset_audits/' . $audit_id);
            ...
            $audit_id= $req->audit_id;
            session()->put('failed','Asset not found');
            return redirect('update_asset_audits/' . $audit_id);
        }

        
    }
    function not_found($audit_id,$asset_id,Request $req)
    {
       
        $unassigned = Asset_audit_master::find($audit_id);
        $u=   $unassigned->not_found;
        $unassigned->not_found = $u.','.$asset_id;
        $unassigned->save();
